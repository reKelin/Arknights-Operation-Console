...
- REQ-PRODUCT-005：用户必须能实时手动录轴，也能在任意帧新增和调整操作点。
  - AC-PRODUCT-005：游戏前台按 P 记录带来源时间证据的待分类操作；Console 前台按 H 整理、Ctrl+S 导出；操作点改帧需要显式确认人工校正。
...
- REQ-PRODUCT-021：代理执行必须视觉定位干员并只使用系统合成输入。
  - AC-PRODUCT-021：部署栏头像唯一匹配后才使用 `InjectTouchInput` 拖拽；技能和撤退只发送用户首次确认过的 Windows 键盘绑定；不读取内存、不注入游戏代码。
- REQ-PRODUCT-022：代理执行必须默认关闭并支持 K 接管和界面停止。
  - AC-PRODUCT-022：每次启动需要明确启用；K、界面停止、离关、失焦、错关或不可信状态立即阻止后续输入并释放活动键与触点。
- REQ-PRODUCT-023：同帧操作必须保持确定顺序和失效安全。
  - AC-PRODUCT-023：按创建顺序和稳定 ID 串行执行；失败后停止该帧剩余操作且不补发。
- REQ-PRODUCT-024：时间轴必须支持高效的横向浏览。
  - AC-PRODUCT-024：滚轮横向滚动，Alt+滚轮围绕指针缩放；同帧操作共享时间位置并纵向堆叠。
- REQ-PRODUCT-025：关卡 OCR 必须覆盖完整黑屏标题画面。
  - AC-PRODUCT-025：连续帧聚合代码和中文名，单独识别到 `OPERATION` 不得提前结束。
- REQ-PRODUCT-026：实时录轴必须支持待分类书签。
  - AC-PRODUCT-026：游戏前台按 P 添加带帧范围和 ClockQuality 的待分类操作，Console 前台按 H 打开集中编辑列表；时间与参数确认前不得导出。
- REQ-PRODUCT-027：代理执行结果必须由游戏观测确认。
  - AC-PRODUCT-027：输入发送成功不得直接记为完成；视觉证据不足时保持暂停并进入待人工确认，回执保留操作、计划帧、观察帧、源时间戳和停止原因。
