...
- REQ-PRODUCT-002：界面必须以计时器和单轨时间轴为核心。
  - AC-PRODUCT-002：默认窗口约 1100×420、宽高可调、最小约 860×340、默认置顶；支持深色与浅色主题。
...
- REQ-PRODUCT-021：代理执行必须视觉定位干员并使用系统合成触摸。
  - AC-PRODUCT-021：部署栏头像唯一匹配后才使用 `InjectTouchInput` 拖拽；不读取内存、不注入游戏代码。
- REQ-PRODUCT-022：代理执行必须默认关闭并支持全局急停。
  - AC-PRODUCT-022：每次启动需要二次确认；F12、离关、失焦、错关或不可信状态立即解除。
...
- REQ-PRODUCT-026：实时录轴必须支持待分类书签。
  - AC-PRODUCT-026：游戏前台按 P 添加当前帧书签，按 H 打开集中编辑列表；书签转换成合法操作前不得导出。
