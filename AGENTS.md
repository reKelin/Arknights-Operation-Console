...
- PR 必须通过 `Smoke` 后才允许合并；不得继续采用“不等 CI 直接合并”的旧约定。创建 PR 不等于授权自动合并。
- PR 冒烟覆盖实际前端基本交互，并包含快速单元测试和静态检查。不能用纯逻辑测试、类型检查或截图替代功能冒烟。
...
- Rust 全量验证、Tauri 绑定生成、完整目录下载、生产构建和原生启动检查保留在独立 Release 流程，仅由版本 tag 或手动触发；私有仓库不运行重型流程，PR 不依赖它。
- 允许本地运行与改动相关的快速检查与测试，按需用 `npm run app:build:local` 编译调试 app；不为一般 PR 擅自运行完整发布构建或触发云端 Release。
- 5/30 分钟只用于观察耗时趋势，不是硬性通过条件。近期连续多次变慢应分析并优化，不删除必要功能检查；防卡死超时另行设置。
