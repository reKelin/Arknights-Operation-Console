...
- AxisLink v1 的事件类型只表示玩家可执行的 `deploy`、`skill`、`retreat`。提醒、到点暂停和自动执行属于 Runner 运行策略，不写入轴事件。
...
- 真实输入默认关闭，必须显式武装。窗口、时钟或状态不可信以及急停触发时立即停止，不盲目补发输入。

## UI 约束

- 只有一个默认约 960×300 的紧凑窗口；宽高均可调整，最小约 760×260。
- 默认置顶，允许从“视图”菜单关闭置顶。
- 最小化进入托盘，关闭窗口直接退出。
